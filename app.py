...
def convert_to_fhir_with_agent(text, max_retries=5, feedback_history=None):
    ...
    if not openai.api_key:
        return {"error": "OpenAI API key not configured", "detailed_logs": []}
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            groundedness_result = evaluate_fhir_groundedness(original_text, fhir_data, max_retries=3, file_hash=file_hash, attempt_number=attempt + 1)
            ...
            # Check if groundedness score is acceptable (5/5)
            groundedness_score = groundedness_result.get("groundedness_score", 0)
            is_acceptable = groundedness_result.get("is_acceptable", False)
            
            # Accept good scores (4+) or when judge explicitly marks as acceptable
            if (groundedness_score >= 5 and is_acceptable) or (groundedness_score >= 4 and is_acceptable):
                logging.info(f"Successfully converted to FHIR on attempt {attempt + 1} with groundedness score {groundedness_score}/5")
                return {
                    "fhir_bundle": fhir_data,
                    "validation": validation_result,
                    "groundedness": groundedness_result,
                    "conversion_notes": f"Document successfully converted to FHIR format with groundedness score {groundedness_score}/5",
                    "detailed_logs": detailed_logs
                }
            else:
                logging.warning(f"Attempt {attempt + 1}: Groundedness score {groundedness_score}/5 is not acceptable. Retrying...")
                
                # Add feedback to history for learning
                feedback_history.append(groundedness_result)
                attempt_log["retry_reason"] = f"Groundedness score {groundedness_score}/5 below threshold"
                
                if attempt == max_retries - 1:
                    return {
                        "error": f"Failed to achieve acceptable groundedness (score: {groundedness_score}/5) after {max_retries} attempts",
                        "validation": validation_result,
                        "groundedness": groundedness_result,
                        "fhir_bundle": fhir_data,
                        "feedback_history": feedback_history,
                        "conversion_notes": f"FHIR conversion completed but groundedness score {groundedness_score}/5 is below acceptable threshold",
                        "detailed_logs": detailed_logs
                    }
                continue
            ...
            if attempt == max_retries - 1:
                return {"error": f"Failed to parse FHIR JSON after {max_retries} attempts: {str(e)}", "detailed_logs": detailed_logs}
            continue
            ...
            if attempt == max_retries - 1:
                return {"error": f"OpenAI API error after {max_retries} attempts: {str(e)}", "detailed_logs": detailed_logs}
            continue
            ...
            if attempt == max_retries - 1:
                return {"error": f"FHIR conversion error after {max_retries} attempts: {str(e)}", "detailed_logs": detailed_logs}
            continue
    
    logging.error("Unexpected end of FHIR conversion function")
    final_result = {"error": "Unexpected error in FHIR conversion", "detailed_logs": detailed_logs}
    logging.info(f"Returning final result with {len(detailed_logs)} detailed logs")
    return final_result
...
def evaluate_fhir_groundedness(original_text, fhir_bundle, max_retries=3, file_hash=None, attempt_number=1):
    ...
    - 5: Excellent - captures all essential information accurately, minor details acceptable to omit
    - 4: Very Good - captures most important information, minor omissions are acceptable  
    - 3: Good - captures core medical facts, some details missing but fundamentally sound
    - 2: Needs Work - missing important information or contains inaccuracies
    - 1: Poor - significant problems with accuracy or completeness
...
def convert_to_fhir(file_hash):
    ...
    try:
        ...
        if "error" in fhir_result:
            return jsonify({"error": fhir_result["error"], "conversion_notes": fhir_result.get("conversion_notes", "")}), 500
        
        return jsonify({"success": True, "conversion_notes": fhir_result.get("conversion_notes", "Successfully converted to FHIR")})
